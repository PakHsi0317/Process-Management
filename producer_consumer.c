...
struct task_struct **cons_process;
struct task_struct *prod_process;
...
static int kthread_consumer (void *id){
    int pointer = *(int *)id;
...
int project_init(void){
    ...
    sema_init(&mutex, 1);
    sema_init(&full, 0);
    cons_process = (struct task_struct**) kmalloc((sizeof(struct task_struct*))*c,GFP_KERNEL) ;
    if(p == 1) prod_process = kthread_run(kthread_producer, NULL, "producer-1");
    
    int i =0;
    for(; i < c;i++){
        cons_process[i] = kthread_run(kthread_consumer,i,"consumer-%d",i);
    }
    return 0;
}
